...
type RestOf<T extends IOrder | NewOrder> = Omit<T, 'placedAt'> & {
...
export class OrderService {
  ...
  protected convertDateFromClient<T extends IOrder | NewOrder | PartialUpdateOrder>(order: T): RestOf<T> {
    return {
      ...order,
      placedAt: order.placedAt?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restOrder: RestOrder): IOrder {
    return {
      ...restOrder,
      placedAt: restOrder.placedAt ? dayjs(restOrder.placedAt) : undefined,
    };
  }

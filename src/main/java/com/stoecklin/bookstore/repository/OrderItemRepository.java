...
public interface OrderItemRepository extends JpaRepository<OrderItem, Long> {
    ...
    @Query(
        value = "select orderItem from OrderItem orderItem left join fetch orderItem.book",
        countQuery = "select count(orderItem) from OrderItem orderItem"
    )
    Page<OrderItem> findAllWithToOneRelationships(Pageable pageable);

    @Query("select orderItem from OrderItem orderItem left join fetch orderItem.book")
    List<OrderItem> findAllWithToOneRelationships();

    @Query("select orderItem from OrderItem orderItem left join fetch orderItem.book where orderItem.id =:id")

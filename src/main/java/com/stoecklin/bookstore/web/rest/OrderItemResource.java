...
/**
 * REST controller for managing {@link com.stoecklin.bookstore.domain.OrderItem}.
 */
@RestController
...
public class OrderItemResource {
     ...
     */
    @GetMapping("/_search")
    public List<OrderItem> searchOrderItems(@RequestParam("query") String query) {
        LOG.debug("REST request to search OrderItems for query {}", query);
        try {
            return StreamSupport.stream(orderItemSearchRepository.search(query).spliterator(), false).toList();
